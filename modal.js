...
modalBtn.addEventListener('click', () => {                       // faire apparaitre le block formulaire au click du modalBtn
...
const validate = (event) => {
  ...
  for (const [key, value] of formData.entries()) {                   // for of pour agir sur plusieur champs sous forme de boucle avec les param key,value
    switch (key) {                                                // switch pour eviter les répétition if if if avec le param key 
      case 'first':                                                 // case qui fonctionne avec switch " dans le cas ou la clef s'apelle first ..ect"
        if (!nameRegex.test(value.trim())) {                      // .trim pour que les espace ne soit pas compter comme des characteres , .test methode de verification du regex 
          showError(firstName, "Veuillez entrer un prénom valide (2 caractères minimum).");     // fonction showError + input + message
          isValid = false;
        } else {
          hideError(firstName);
        }
      ...
      case 'last':
        if (!nameRegex.test(value.trim())) {
          showError(lastName, "Veuillez entrer un nom valide (2 caractères minimum).");
          isValid = false;
        } else {
          hideError(lastName);
        }
...
// j'utilise Array.from location( mes boutons radios ) pour les intégrer dans un tableau et avec .some je verifie que au moin 1 data du tableau est checked                              
    ...
    const locationChecked = Array.from(locations).some(button => button.checked);    
  ...
  if (!conditions.checked) {    // ici on gere la case conditions qui a comme indication html "checked" , si la case n'est pas checked , error.
...
closeBtn.addEventListener('click', () => {                       // On ferme la modal avec la croix 
  modalbg.style.display = "none";
  form.reset();
  hideAllErrors()
});

closeValidated.addEventListener('click', () => {                       // On ferme la modal avec la croix 
